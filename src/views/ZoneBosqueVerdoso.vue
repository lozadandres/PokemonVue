<template>
  <div class="gym-zone">
    <div class="gym-header">
      ...
      <div class="gym-info">
        <h1>Bosque Verdoso</h1>
        <p>Tipo: Planta/Bicho</p>
      </div>
      <div class="stats-bar">
    ...
    <div class="gym-map-container">
      <div ref="mapCanvas" class="map-canvas">
        <img ref="mapImg" :src="zoneImage" alt="Bosque Verdoso" class="map-image" />
        ...
        <div class="npc trainer" :style="trainer1Style" v-if="!trainersDefeated.trainer1">
          <div class="npc-sprite"><img :src="playerImage" alt="Cazabichos José" class="npc-image small" /></div>
          <div class="npc-label trainer">Cazabichos José</div>
        </div>

        <div class="npc trainer" :style="trainer2Style" v-if="!trainersDefeated.trainer2">
          <div class="npc-sprite"><img :src="playerImage" alt="Jardinera Rosa" class="npc-image small" /></div>
          <div class="npc-label trainer">Jardinera Rosa</div>
        </div>
      </div>
    </div>
    ...
    >
        <div v-if="!showRoute">
            <p>
                ¡Bienvenido, joven entrenador! Estás en el <strong>Bosque Verdoso</strong>, un laberinto natural donde los Pokémon bicho y planta reinan supremos.
            </p>
            <p>
                Para demostrar tu valía, debes capturar 2 especies nativas y encontrar 2 muestras biológicas. Solo entonces podrás desafiarme.
            </p>
        </div>
        <div v-else>
            <p>
                ¡Veo que has aceptado el reto! La naturaleza te observa.
            </p>
            <p>
                Completa tus objetivos y regresa cuando estés listo. ¡Confío en tu potencial!
            </p>
        </div>
    </GameLeaderDialog>
    ...
    <div v-if="currentEncounter?.type === 'pokemon'" class="modal-overlay">
      <div class="modal pokemon-modal">
        ...
        <div class="modal-content">
          ...
          <p class="encounter-text">Un {{ currentEncounter.data.name }} apareció entre la hierba alta.</p>
        </div>
    ...
    <div v-if="showRoute" class="route-overlay">
        <div class="route-marker" style="top: 30%; left: 40%">
            <div class="pokeball-icon"></div>
            <span class="marker-label">Heracross</span>
        </div>
        <div class="route-marker" style="top: 60%; left: 70%">
            <div class="pokeball-icon"></div>
            <span class="marker-label">Pinsir</span>
        </div>
        <div class="route-marker" style="top: 20%; left: 80%">
            <div class="pokeball-icon"></div>
            <span class="marker-label">Bulbasaur</span>
        </div>
    </div>
...
import zoneImage from '../assets/zona/bosqueverdoso.png'
...
const wildPokemon = [
  { id: 1, name: 'Caterpie', level: 4, found: false },
  { id: 2, name: 'Weedle', level: 4, found: false },
]

const items = [
  { id: 1, name: 'Poción', description: 'Restaura 20 PS.', found: false },
  { id: 2, name: 'Antídoto', description: 'Cura el envenenamiento.', found: false },
]
...
const checkCompletion = () => {
  if (pokemonCaptured.value >= 2 && itemsFound.value >= 2) {
    mapStore.completeZone(1) // Bosque Verdoso ID
...
const trainers = {
  trainer1: { name: 'Cazabichos José', dialogue: '¡Me gustan los bichos!', pokemon: 'Metapod', pokemonLevel: 5, position: { x: 30, y: 55 } },
  trainer2: { name: 'Jardinera Rosa', dialogue: 'Las flores son hermosas.', pokemon: 'Oddish', pokemonLevel: 6, position: { x: 65, y: 40 } },
}

// Leader Position
const leaderPos = { x: 80, y: 20 }
...
const goBack = () => { router.push('/zone/1') }
</script>

<style scoped>
.gym-zone{width:100%;min-height:100vh;background:linear-gradient(135deg,#2e7d32 0%,#66bb6a 50%,#2e7d32 100%);position:relative;overflow:hidden}
...
.route-marker {
    ...
    align-items: center;
    z-index: 500; /* High z-index ensured */
