<template>
  <div class="lobby-view">
    ...
    <div class="lobby-map-container">
        <img :src="lobbyImage" alt="Lobby Cueva Ignea" class="lobby-bg" />
    ...
    >
        <p>¡Bienvenido al calor de la Cueva Ígnea! Soy Vulcano.</p>
        <p>Aquí solo los fuertes sobreviven. Demuestra tu valor capturando 2 Pokémon de Fuego y recuperando 2 Rocas Magmáticas.</p>
    </LeaderInteractionModal>
    ...
    <div v-if="isSearching" class="searching-overlay">
        <div class="searching-content">
            ...
            <h2>Buscando Pokémon de Fuego...</h2>
            <p>El calor es intenso...</p>
        </div>
    </div>
...
import { ref } from 'vue';
import lobbyImage from '../assets/loby/lobycuevaignea.png';
...
const npcs = ref<LobbyNpc[]>([
    { id: 2, name: 'Montañero Bruno', role: 'Entrenador' },
    { id: 3, name: 'Domadora Flavia', role: 'Entrenador' },
    { id: 4, name: 'Científico Loco', role: 'Investigador' },
]);
const onlinePlayers = ref([{ id: 1, name: 'Blaine', level: 45 }, { id: 2, name: 'Flannery', level: 38 }]);

const addVisitor = () => {
    npcs.value.push({
        id: Date.now(),
        name: `Explorador ${Math.floor(Math.random() * 100)}`,
        role: 'Aventurero'
    });
};
...
const startAsyncSearch = () => {
    ...
    setTimeout(() => {
        ...
        router.push({ name: 'game-cueva', query: { showRoute: 'true' } });
    }, 3000);
};
...
.lobby-header h1 { color: white; background: rgba(191, 54, 12, 0.8); padding: 10px 20px; border-radius: 8px; margin: 0; box-shadow: 0 4px 6px rgba(0,0,0,0.3); }
...
.action-btn-small:hover { background: #e64a19; }
...
input:checked + .slider { background-color: #d84315; }
